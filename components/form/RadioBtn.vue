            ...
            <div class="flex gap-2 lg:gap-x-4 lg:gap-y-3 flex-wrap">
                <div v-for="(option, i) in options" :key="i" class="flex-1" @click.prevent="$emit('emit', option.emit)">
                    <span class="group relative">
                        <label
                            class="relative border border-slate-300 py-3 rounded font-semibold text-base xl:text-lg !pl-12 pr-6 group-hover:bg-accent-50 group-hover:bg-opacity-30 hover:border-accent-400 cursor-pointer flex"
                        ...
                        >
                            ...
                            <div
                                class="absolute left-4 self-center transition-all duration-75 peer-checked:border-accent-500 peer-checked:bg-accent-500 group-hover:bg-accent-200 h-5 w-5 rounded-full outline-2 outline outline-slate-200 peer-checked:outline-accent-500 group-hover:outline-accent-500 outline-offset-[0px] group-hover:outline-offset-[3px] peer-checked:outline-offset-[3px] ring-white"
                            ></div>
                        </label>
                    </span>
                </div>
